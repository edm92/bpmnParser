...
import org.yaoqiang.bpmn.model.elements.collaboration.*;
import org.yaoqiang.bpmn.model.elements.activities.*;
	...
	public static Graph<Vertex,Edge> loadModel(String filename, int flags){
		...
		if(document != null) {
			...
			for(String key : tEdges.keySet()){
				...
				if(!tNodes.containsKey(r.src)) { System.out.println("Unable to find vertex reference ID " + r.src); continue; };
				if(!tNodes.containsKey(r.trg)) { System.out.println("Unable to find vertex reference ID " + r.trg); continue; };
				if(tNodes.get(r.src).node == null) {System.out.println("Node not added ID " + r.src); continue; };
				if(tNodes.get(r.trg).node == null) {System.out.println("Node not added ID " + r.trg); continue; };
				// Create a new edge
...
class TEdge {
	...
	public TEdge(String _id, String _name, String _src, String _trg, int _type, XMLElement _ele){
		id = _id;name = _name;src = _src;trg = _trg;type = _type;ele = _ele; }
}
class TNode {
	String id;
	String name;
	...
	public TNode(String _id, String _name, int _type, XMLElement _ele){
		id = _id; 	name = _name;	type = _type;	ele = _ele;
	}
